...
|---|---|---|---|---|---|---|
| Text generation | Gemini (primary), DeepSeek (fallback), Ollama (opt-in) | Gemini/DeepSeek working; Ollama not installed on dev machine yet | Ollama (open-weight models, FREE API — local, no cost) | Ollama on M1 now, RTX 5060 Ti later | Gemini, DeepSeek | Ollama once installed; keep Gemini/DeepSeek as cloud fallback |
